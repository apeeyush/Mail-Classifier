...
count_number=0
count_dollar=0
mypath="/home/hardy/Projects/ML_Project/classification 2/Advertisements publishers"
for fk in listdir(mypath):
    if isdir(join(mypath,fk)):
        mypath_2=join(mypath,fk)
        os.chdir(mypath_2)
        for fo in listdir(mypath_2):
            if isfile(join(mypath_2,fo)):
##                print join(fk,fo)
                f=open(fo,"rw+")
                fr=f.read()
                subl=[]
